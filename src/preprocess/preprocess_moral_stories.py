...
def main() -> None:
    base = _common.RAW_ROOT / "moral_stories-master"
    if not base.exists():
    ...
    if not data_dir.exists():
        print("No data directory found under moral_stories-master. Dataset files appear missing.")
        return

    files = _collect_files(data_dir)
    if not files:
        print("No dataset files found under moral_stories-master/data")
        return
    ...
    for path in files:
        ...
        out_base = (_common.PROCESSED_ROOT / "moral_stories" / rel).with_suffix("")
